    ...
    static void dfs(int u) {
        ...
        for(int v : adjList[u])
            if(dfs_num[v] == 0) {
                ...
                dfs(v);
                if(dfs_low[v] >= dfs_num[u]) artPoints[u] = true;
                if(dfs_low[v] > dfs_num[u]) System.out.printf("Bridge between %d %d%n", u, v);
                dfs_low[u] = Math.min(dfs_low[v], dfs_low[u]);
            } else if(parent[u] != v) dfs_low[u] = Math.min(dfs_low[u], dfs_num[v]);
    }
}
